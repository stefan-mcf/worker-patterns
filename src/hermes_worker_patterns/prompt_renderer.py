...
def render_prompt_contract(plan: PatternPlan) -> str:
    ...
    return "\n".join(
        [
            ...
            *lane_sections,
            (
                "Dry-run only: render Hermes-compatible prompts/specs, but do not "
                "launch workers, create Kanban tasks, or mutate Hermes settings."
            ),
        ]
    )
...
def render_prompt_bundle(plan: PatternPlan) -> dict[str, object]:
    return {
        ...
        "primary_mechanism": plan.hermes_mapping.primary_mechanism.value,
        "fallback_mechanisms": [
            mechanism.value for mechanism in plan.hermes_mapping.fallback_mechanisms
        ],
        "overlays_summary": _overlays_summary(plan),
        "invocation_hint": plan.hermes_mapping.invocation_hint,
