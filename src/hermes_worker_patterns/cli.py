...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Select Hermes worker patterns and render dry-run execution specs."
    )
    sub = parser.add_subparsers(dest="command", required=True)

    for name in REQUEST_COMMANDS:
        help_text = "select a worker pattern and Hermes mapping"
        if name.startswith("render-") or name == "render":
            help_text = "render dry-run Hermes prompt/spec output"
...
hermes_primary: {mapping.primary_mechanism.value}
hermes_hint: {mapping.invocation_hint}
lanes:
{lanes}
proof_expectations:
{proof}
