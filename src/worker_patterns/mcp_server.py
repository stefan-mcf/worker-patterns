...
from .runtime_tool import worker_pattern_tool
...
def select_worker_pattern_bridge(args: JsonArgs) -> dict[str, Any]:
    ...
    intentionally pure: no worker launch, no Kanban writes, no runtime config
...
def render_execution_plan_bridge(args: JsonArgs) -> dict[str, Any]:
    """Return the dry-run runtime execution plan as JSON-safe data.
...
def create_mcp_server() -> Any:
    ...
    server = FastMCP("worker-patterns")
        ...
        if method == "initialize":
            return _json_rpc_result(
                request_id,
                {
                    ...
                    "serverInfo": {"name": "worker-patterns", "version": "0.1.0"},
                    "capabilities": {"tools": {}},
                },
            )
    ...
    return [
        {
            ...
            "description": "Select a dry-run-safe worker pattern.",
            "inputSchema": schema,
        },
        {
            "name": "render_execution_plan",
            "description": "Render a dry-run runtime execution plan for a selected worker pattern.",
            "inputSchema": schema,
        },
    ]
